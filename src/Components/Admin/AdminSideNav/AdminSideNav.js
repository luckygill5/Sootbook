...
function AdminSideNav(props) {
    ...
    function handleMenuItem(event, name, subMenu = false) {
        ...
        if (document.querySelector(`.${name}`)?.parentElement?.parentElement.className == 'subMenu') {
            let breadcrumbData = [];
            breadcrumbData.push(
                document.querySelector(`.${name}`).parentElement?.parentElement?.previousElementSibling.querySelector('.text').innerHTML,
            );
                            ...
                            {sidebarMenu?.length > 0 &&
                                sidebarMenu.map((item, index) => {
                                    return (
                                        <li key={index}>
                                            <button
                                                className={`menu_link ${item.item == menuSelected && 'active'}`}
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                <div className='icon'>{item.icon}</div> <span className='text'>{item.item}</span>
                                                {item?.childItem?.subMenu && item?.childItem?.subMenu.length > 0 && (
                                            ...
                                            {(item.item == menuSelected || item.item == subMenuParent) &&
                                                ...
                                                item?.childItem?.subMenu.length > 0 && (
                                                    <ul className='subMenu'>
                                                        {item?.childItem?.subMenu.map((label, index) => {
                                                            return (
                                                                <li>
                                                                    <button
